...
- **使用场景：** 数组全局有序，且元素不重复。要求寻找某个确切的 `target`。



#### 2. 寻找左右边界

- **使用场景：** 数组全局有序，但**包含重复元素**。要求寻找 `target` 第一次出现（左边界）或最后一次出现（右边界）的位置。（如 LeetCode 34）
...
- **使用场景：** 数组“局部有序”（如旋转数组）或者存在“高低起伏”（如寻找峰值）。（如 LeetCode 153、33）
